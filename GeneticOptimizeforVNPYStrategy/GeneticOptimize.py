...
def object_func(strategy_avg):
    ...
    engine.setStartDate('20150901')                 # 设置回测用的数据起始日期
    engine.setEndDate('20160201')                   # 设置回测用的数据起始日期
    ...
    # result = engine.calculateDailyStatistics()[1]

    winningRate = round(backresult['winningRate'],3)  #收益回撤比
    capital= round(backresult['capital'],3)                   #夏普比率                 #夏普比率
    profitLossRatio= round(backresult['profitLossRatio'],3)
    return capital,winningRate,profitLossRatio
...
def optimize():
    ...
    # 遗传算法参数设置
    MU = 30  # 设置每一代选择的个体数
    LAMBDA = 30 # 设置每一代产生的子女数
    pop = toolbox.population(50)  # 设置族群里面的个体数量
    CXPB, MUTPB, NGEN = 0.5, 0.3, 15  # 分别为种群内部个体的交叉概率、变异概率、产生种群代数
