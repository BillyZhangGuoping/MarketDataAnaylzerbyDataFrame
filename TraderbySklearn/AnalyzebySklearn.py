...
if __name__ == '__main__':
    ...
    #数据库导入
    start = datetime.strptime("20181001", '%Y%m%d')
    end = datetime.strptime("20190501", '%Y%m%d')
    df = DA.db2df(db="VnTrader_1Min_Db", collection="rb1910", start = start, end = end)
    ...
    X_train_new = selectPer.fit_transform(X_train, y_train)
    print(X_train_new.shape)
    X_test_new = selectPer.transform(X_test)
    ...
    param_grid_logit= {'solver': ['liblinear','lbfgs','newton-cg','sag'],'multi_class':['auto']}
    ...
    clf_selected=SVC(C=1, kernel='rbf', gamma=4) #此处填入网格回测最优模型和参数
